...
`StyledTextKit` is a declarative building and fast rendering 
attributed string library. It serves as a replacement to 
`NSAttributedString` and `UILabel` for background-thread 
size and bitmap caching.
...
The project has been used by `hiddenmemory` in a number of projects. The original upstream has become stagnant and unmaintained. As
we are moving projects to Swift Package Manager, it was felt easier to maintain our own version of this code until a point where we
no longer need it in our projects.
